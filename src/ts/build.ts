...
async function buildPage(config: PageConfig, assets: SharedAssets) {
  ...
  if (config.spreadPhotos) {
    for (const sp of config.spreadPhotos) {
      ...
      if (sp.label) {
        ...
        ctx.fillStyle = sp.labelColor ?? '#1a3a5c';
  ...
  if (config.trajectories) {
    for (const traj of config.trajectories) {
      ...
      if (traj.arrowEnd) {
        ...
        ctx.lineTo(last.x - alen * Math.cos(ang - 0.4), last.y - alen * Math.sin(ang - 0.4));
        ctx.moveTo(last.x, last.y);
        ctx.lineTo(last.x - alen * Math.cos(ang + 0.4), last.y - alen * Math.sin(ang + 0.4));
        ctx.stroke();
      }
  ...
  if (config.trackingLabels) {
    for (const tl of config.trackingLabels) {
      ...
      const maxW = Math.max(...allLines.map(l => l.length)) * fontSize * 0.55 + padX * 2;
      ...
      ctx.strokeStyle = accent;
      ctx.lineWidth = 1.2;
      [[tl.x, tl.y], [tl.x + maxW, tl.y], [tl.x, tl.y + boxH], [tl.x + maxW, tl.y + boxH]].forEach(([cx, cy], i) => {
        const sx = i % 2 === 0 ? 1 : -1;
        const sy = i < 2 ? 1 : -1;
        ctx.beginPath();
      ...
      (tl.data ?? []).forEach((line, i) => {
        ctx.fillText(line, tl.x + padX, tl.y + padY + fontSize + (dataStart + i) * lineH);
      });
  ...
  if (config.annotations) {
    for (const ann of config.annotations) {
      ...
      // Draw bounding box
      const annColor = ann.color ?? '#00ff00';
