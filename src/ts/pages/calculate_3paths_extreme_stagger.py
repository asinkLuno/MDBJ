...
# === 几何定义 (延迟收束：号角形) ===
spine = [(100, 850), (550, 850), (1000, 450), (1320, 80)]

# 【关键调整】把 P1 和 P2 往外大幅拉扯，让前半段保持极宽，最后 1/3 才急剧收拢
# 左墙 (向上扩张)：中间的点推高到了 Y=300 和 Y=50
left_wall = [(-100, 450), (200, 300), (1050, 50), (1320, -20)]
# 右墙 (向下扩张)：中间的点推低到了 Y=1300 和 Y=850
right_wall = [(100, 1250), (500, 1300), (1150, 850), (1320, 180)]
...
def add_bezier_wall(space, p0, p1, p2, p3):
    steps = 15
    prev = (bezier(0, p0[0], p1[0], p2[0], p3[0]), bezier(0, p0[1], p1[1], p2[1], p3[1]))
    for i in range(1, steps + 1):
        t = i / steps
        curr = (bezier(t, p0[0], p1[0], p2[0], p3[0]), bezier(t, p0[1], p1[1], p2[1], p3[1]))
...
cap_top = pymunk.Segment(space.static_body, (1320, -20), (1320, 180), 20)
space.add(cap_top)
...
for i, (city, dates) in enumerate(all_labels):
    ...
    lx = bezier(t_spawn, left_wall[0][0], left_wall[1][0], left_wall[2][0], left_wall[3][0])
    ly = bezier(t_spawn, left_wall[0][1], left_wall[1][1], left_wall[2][1], left_wall[3][1])
    rx = bezier(t_spawn, right_wall[0][0], right_wall[1][0], right_wall[2][0], right_wall[3][0])
    ry = bezier(t_spawn, right_wall[0][1], right_wall[1][1], right_wall[2][1], right_wall[3][1])
...
for p in planes_data:
    ...
    # 【改动】放宽 Y 轴的下边界到 1200，保证散得极开的飞机也不会被裁掉切平
    px = max(20, min(1320, px))
    py = max(20, min(1200, py))
...
planes_block = "const planes: SpreadPhotoLayout[] = [\n" + "\n".join(plane_lines) + "\n];"
