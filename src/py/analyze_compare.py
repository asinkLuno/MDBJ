...
try:
    ...
    def analyze_file(file_path, _label):
        ...
        matched_dates = dates_raw["matched_date"].dropna()
        performance = hs300_df[hs300_df["date"].isin(matched_dates)]["pctChg"].dropna()  # type: ignore
        return performance
...
def get_stats(perf, all_p):
    _t_stat, p_val = stats.ttest_ind(perf, all_p, equal_var=False)
    return {
        "样本数": len(perf),
...
df_compare = pd.DataFrame(
    {
        ...
        "全样本 (基准)": [
            len(all_perf),
            f"{all_perf.mean():.4f}%",
            f"{all_perf.median():.4f}%",
            f"{(all_perf > 0).mean()*100:.2f}%",
            f"{all_perf.std():.4f}",
            "-",
        ],
        "红歌日期 (Red)": [
            stats_red[k]
            for k in ["样本数", "平均涨跌幅", "中位数", "上涨概率", "标准差", "P-值"]
        ],
        "绿歌日期 (Green)": [
            stats_green[k]
            for k in ["样本数", "平均涨跌幅", "中位数", "上涨概率", "标准差", "P-值"]
